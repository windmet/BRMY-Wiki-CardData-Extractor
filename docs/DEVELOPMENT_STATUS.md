...
> **文档状态说明（2026-08-15）**：本文主体是 2026-07-16 旧工作目录的历史快照，旧 EXE 哈希和菜单 1 至 12 只用于回溯，不能代表当前独立仓库发布状态。当前分支有 75 项自动化测试，已完成 `home_voices` 的 2099 行 ACB 拉表、三轮生日台词选择、近 365 天主页/季节与生日祝福双分表，以及保留 Wiki 人工列的卡牌 Update/Diff Mode。主页语音 XLSX 只保留 Wiki 编辑列，技术审计输出到 JSON/Markdown。受控 Windows 构建、SHA/manifest 和冻结 EXE 烟雾已实现；根目录的 EXE 仍是本地构建产物，尚未发布 GitHub Release。
...
- 所有 masterdata domain 均已使用 `TableCatalog` 按表名读取；活动版本相关扩展表通过 `required=False` 保持可选兼容。
...
按角色生日与周年边界自动选择最新轮次，也支持 `--cycle` / `--year` 覆盖。第一、二轮的 5 条与第三轮的 3 条分别按实际行数导出；原始 `CharacterBirthdayTextNo` 和 `KeyTargetValue` 保存在 JSON 审计元数据中。角色 ID 映射覆盖 1 至 21，详见 [`BIRTHDAY_CYCLES.md`](BIRTHDAY_CYCLES.md)。
...
6. **活动机制仍需持续建模**：识表方式已统一，但新 `EventFormat`、奖励枚举和扩展表语义仍需随游戏更新增加夹具与字段说明。
...
10. **自动化测试仍需继续扩展**：当前 59 项已覆盖解码缓存、schema/session、具名表撞名、生日三种编号与交互选择、Excel 列类型、卡牌关系、活动奖励和 ACB 关联；各 domain 的新机制与更新差分仍需持续增加夹具。
